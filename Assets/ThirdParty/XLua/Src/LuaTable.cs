...
namespace XLua
{
    public partial class LuaTable : LuaBase
    {
        ...
        // no boxing version get
        public void Get<TKey, TValue>(TKey key, out TValue value, out LuaTypes lua_type)
        {
#if THREAD_SAFE || HOTFIX_ENABLE
            lock (luaEnv.luaEnvLock)
            {
        ...
        public void ForEach<TKey, TValue>(Action<TKey, TValue, LuaTypes> action)
        {
#if THREAD_SAFE || HOTFIX_ENABLE
            lock (luaEnv.luaEnvLock)
            {
