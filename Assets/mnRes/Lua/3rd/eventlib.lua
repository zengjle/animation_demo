...
function _M:Connect(handler, from)
    ...
    table.insert(self.handlers, {func = handler, from = from})
    local t = { }
    t.func = handler
...
function _M:Disconnect(handler, from)
    assert(self ~= nil and type(self) == "table", "Invalid Event (make sure you're using ':' not '.')")
    ...
    if not handler then
        ...
        for k, v in pairs(self.handlers) do
            if v.func ==  (type(handler) == "table" and handler.func or handler) and v.from == from then
                self.handlers[k] = nil
                return k
            end
        end
...
function _M:Fire(filter, ...)
    ...
    for k, v in pairs(self.handlers) do
        i = i + 1
        if not filter or (filter and not filter(v)) then
            if v.from then
                spawn(function() 
                    v.func(v.from, unpack(self.args)) 
                    i = i - 1
